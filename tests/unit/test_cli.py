...
# ``score`` (issue #10), and ``generate`` (issue #12) became real commands, so
...
# test_corpus.py, test_score_cli.py, and test_generate.py). Flag-rejection
# (argparse, pre-handler) still covers every command. The rest remain stubs
# until their own tickets.
STUB_COMMANDS = sorted(set(cli.COMMANDS) - {"build-corpus", "score", "generate"})
